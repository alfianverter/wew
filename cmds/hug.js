...
module.exports.run = (client, message, args) => {
    ...
    if (!args[0]) {
        const ghembed = new Discord.RichEmbed()
            .setColor(`RANDOM`)
            .setDescription(`Ooppss! **${message.author.username}** is hugging themselves! `)
            .setImage(hug[hugresult])
            .setFooter(`Request by : ${message.author.username} | © Sharif#2769`);
        message.channel.send({
            embed: ghembed
        })
        return;
    }
    if (!message.mentions.members.first() === message.isMentioned(message.author)) {
        const hembed = new Discord.RichEmbed()
            .setColor(`RANDOM`)
            .setDescription(`**${message.author.username}** hugging **${message.mentions.members.first().user.username}** so hot!`)
            .setImage(hug[hugresult])
